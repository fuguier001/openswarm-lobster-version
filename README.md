...
### 必需
- ✅ **OpenClaw** - 已安装并正常运行
- ✅ **Waza skill** - 工程习惯技能库（pair-coding 依赖）
- ✅ **ChromaDB** - 向量数据库（cognitive-memory 依赖）

### 可选
- 🐳 **Docker** - 用于运行 ChromaDB（推荐）
- 🐍 **Python 3.8+** - 用于本地安装 ChromaDB

### 安装 Waza

#### 什么是 Waza？
Waza 是一个工程习惯技能库，包含 8 个核心技能（think, design, check, hunt, write, learn, read, health）。pair-coding skill 依赖 Waza 的 **check** skill 进行代码审查。

#### 检查 Waza 是否已安装
```bash
# 检查 Waza skill 是否存在
ls ~/.npm-global/lib/node_modules/openclaw/skills/waza/
...
```bash
# 查看所有 Waza 技能
ls ~/.npm-global/lib/node_modules/openclaw/skills/waza/skills/

# 预期输出：check design hunt health learn read think write
```

如果 Waza 未安装，pair-coding skill 会在审查步骤报错。
...
如果 ChromaDB 未运行，cognitive-memory skill 会提示错误，但 pair-coding 和 code-registry 仍然可以正常使用。
